...
public class BusinessDetailsActivityTest extends ActivityInstrumentationTestCase2<BusinessDetailsActivity> {

  private static final int FAKE_USER_ID = 10;

  private BusinessDetailsActivity businessDetailsActivity;

  public BusinessDetailsActivityTest() {
    super(BusinessDetailsActivity.class);
  }

  @Override protected void setUp() throws Exception {
    super.setUp();
    this.setActivityIntent(createTargetIntent());
    this.businessDetailsActivity = getActivity();
  }
  ...
  public void testContainsUserDetailsFragment() {
    Fragment userDetailsFragment =
        businessDetailsActivity.getFragmentManager().findFragmentById(R.id.fl_fragment);
    assertThat(userDetailsFragment, is(notNullValue()));
  }

  public void testContainsProperTitle() {
    String actualTitle = this.businessDetailsActivity.getTitle().toString().trim();
  ...
  private Intent createTargetIntent() {
    Intent intentLaunchActivity =
        BusinessDetailsActivity.getCallingIntent(getInstrumentation().getTargetContext(), FAKE_USER_ID);
